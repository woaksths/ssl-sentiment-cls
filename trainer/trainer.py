...
class Trainer(object):
    ...
    def _train_batch(self, input_variable, input_lengths, target_variable, model):
        loss = self.loss
        # Forward propagation
        print('loss', loss)
        print('input_variables', input_variable)
        print('input_lengths', input_lengths)
        
        output = model(input_variables, input_lengths)
        # Get loss
        loss.reset()
    ...
    def _train_epoches(self, data, model, n_epochs,
                       start_epoch, start_step, dev_data=None):
        log = self.logger
        ...
        print('step', step)
        step_elapsed = 0
        print('step_elapsed', step_elapsed)
        for epoch in range(start_epoch, n_epochs + 1):
            log.debug("Epoch: %d, Step: %d" % (epoch, step))
            
            batch_generator = batch_iterator.__iter__()
            # consuming seen batches from previous training
            print('check', (epoch-1) * steps_per_epoch)
            print('step', step)
